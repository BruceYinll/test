 ...
 */
@SuppressLint("NewApi")
public class ModifyBaseAsyncTask<T> extends AsyncTask<HashMap<String, String>, Integer, BaseModel<T>> {
	 ...
	 *            接口编码
	 */
	public ModifyBaseAsyncTask(Context ctx, Type t, int inf) {
		this(ctx, t, inf, true);
	}
	 ...
	 *            是否显示进度条
	 */
	public ModifyBaseAsyncTask(Context ctx, Type t, int inf, boolean isShow) {
		mContext = ctx;
		InfCode = inf;
		mType = t;
		mIsShow = isShow;
	...
	public ModifyBaseAsyncTask(Context ctx, Type t, int inf, boolean isShow,Object obj) {
		mContext = ctx;
		InfCode = inf;
		mType = t;
		mIsShow = isShow;
	 ...
	 *            接口编码
	 */
	public ModifyBaseAsyncTask(BaseFragment baseFragment, Type t, int inf) {
	...
	public ModifyBaseAsyncTask(BaseFragment baseFragment, Type t, int inf, boolean isShow) {
		fragment = baseFragment;
		InfCode = inf;
		isFragment = true;
		mType = t;
	...
	protected BaseModel<T> doInBackground(HashMap<String, String>... params) {
		...
		case InterfaceFinals.INF_GET_QUESTION_LIST_UPDATA: // 获取运维列表
			suffix = "/m/question/findQuestionList?";
			resModel = postData(suffix, params[0]);
			break;
		...
		case InterfaceFinals.INF_APPVERSION:// 版本更新
			suffix = "/m/appVersion/updateVersionInfo?";
			resModel = postData(suffix, params[0]);
			break;
		...
		if (!HttpUtil.isCancelled()) {// 未终止
			if (isFragment) {
				...
				if (isException) {// 解析异常
					...
					((BaseInteractActivity) mContext).onSuccess(result,obj);
				} else {// fail
