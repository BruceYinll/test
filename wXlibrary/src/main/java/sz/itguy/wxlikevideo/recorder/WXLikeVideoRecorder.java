...
public class WXLikeVideoRecorder implements Camera.PreviewCallback, CameraPreviewView.PreviewEventListener {
	...
	// 最长录制时间30秒
	private static final long MAX_RECORD_TIME = 1000 * 30;
	// 帧率
	...
	private void initRecorder() {
		...
		RecorderParameters recorderParameters = RecorderParameters
				.getRecorderParameter(Constants.RESOLUTION_MEDIUM_VALUE);
		strFinalPath = FileUtil.createFilePath(mFolder, null, Long.toString(System.currentTimeMillis()));
		// recorder = new FFmpegFrameRecorder(strFinalPath, imageWidth,
		// imageHeight, 1);
		// 初始化时设置录像机的目标视频大小
	...
	// strVideoPath = Util.createFilePath(mFolder, "tmp",
	// Long.toString(System.currentTimeMillis()));
	// RecorderParameters recorderParameters =
	// Util.getRecorderParameter(currentResolution);
	...
	private void initFrameFilter() {
		if (TextUtils.isEmpty(mFilters)) {
			mFilters = generateFilters((int) (1f * outputHeight / outputWidth * imageHeight), imageHeight, 0, 0,
					"clock");
		}
		mFrameFilter = new FFmpegFrameFilter(mFilters, imageWidth, imageHeight);
		mFrameFilter.setPixelFormat(org.bytedeco.javacpp.avutil.AV_PIX_FMT_NV21); // default
																					// camera
																					// format
																					// on
																					// Android
	...
	public final int AUDIO_ERRO = 6;
	public final int AUDIO_START = 5;
	...
	Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what == AUDIO_ERRO) {
				if (recordListener != null) {
					recordListener.error();
					strFinalPath = null;
				}
			} else if (msg.what == AUDIO_START) {
				try {
	...
	class AudioRecordRunnable implements Runnable {
		...
		public void run() {
			...
			try {
				...
				bufferSize = AudioRecord.getMinBufferSize(SAMPLE_AUDIO_RATE_IN_HZ, AudioFormat.CHANNEL_IN_MONO,
						...
						if (recording) {
							if (RECORD_LENGTH <= 0)
								try {
									...
									// Log.v(LOG_TAG,"recording " + 1024*i + "
									// to "
									// + 1024*i+1024);
								} catch (FFmpegFrameRecorder.Exception e) {
									Log.v(TAG, e.getMessage());
	...
	public static interface RecordListener {
		abstract void onAudioStart();

		abstract void onAudioStop();
