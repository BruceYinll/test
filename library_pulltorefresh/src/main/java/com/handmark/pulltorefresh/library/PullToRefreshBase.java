...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private int mTouchSlop; // 表示一个距离，表示滑动的时候，手的移动要大于这个距离才开始移动控件
	private float mLastMotionX, mLastMotionY;
	private float mInitialMotionX, mInitialMotionY;

	private boolean mIsBeingDragged = false; // PullRefreshView是否正在被拉拽
	private State mState = State.RESET;
	// PullRefreshView目前的状态 有以下几种状态
	// RESET 用户没有使用
	// PULL_TO_REFRESH 用户下拉了一点点
	// RELEASE_TO_REFRESH 用户已经下拉到了一旦释放就能引发刷新(加载更多)事件
	// REFRESHING PullRefreshView正在刷新中
	// MANUAL_REFRESHING 由程序引发的正在刷新中
	// OVERSCROLLING
	// 用户手势对ScrollView进行了一甩的操作，ScrollView会有一弹的效果，PullRefreshView正处于这个状态

	private Mode mMode = Mode.getDefault();
	// 用户选择的模式，mMode, 默认的是PULL_FROM_START
	// DISABLED 不允许
	// PULL_FROM_START 只允许Refreshable View从start开始刷新，根据横纵向，从上或者左开始可以刷新
	// PULL_FROM_END 只允许从底部开始拉
	// BOTH 既可以从上面开始往下拉，也可以从下面开始往上拉
	// MANUAL_REFRESH_ONLY 不允许拉动刷新，但是支持代码控制的刷新
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (isReadyForPull()) {
				...
				// direction
				switch (getPullToRefreshScrollDirection()) { // 获取继承这个PullToRefreshBase的View的方向，横向或者纵向
				case HORIZONTAL: // 如果为横向的话
					diff = x - mLastMotionX;
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			 ...
			 * 
			 * @yeonkitt@gmail.com dead code, no need to modify.
			 */
	...
	private void init(Context context, AttributeSet attrs) {
		...
		ViewConfiguration config = ViewConfiguration.get(context); // 存储的是系统自带的各种final数据,比如滚动的时间之类的
		mTouchSlop = config.getScaledTouchSlop(); // 获取一个距离，表示滑动的时候，手的移动要大于这个距离才开始移动控件

		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, // 存储的是从xml文件中获取的各项参数,ptr:开头
				R.styleable.PullToRefresh);

		if (a.hasValue(R.styleable.PullToRefresh_ptrMode)) {
			// ptr:ptrMode
			// disabled 不支持刷新
			// pullFromStart 只支持从头开始刷新
			// pullFromEnd 只支持从底部开始刷新
			// both 从头和为开始刷新都支持
			// manualOnly 不支持手动的刷新 只支持代码控制的刷新
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrAnimationStyle)) {
			...
			// rotate 图片随着用户打动的距离滚动
			// flip 用户拉在状态下拉一点点时显示的是正的，当用户拉到下拉刷新状态时上下翻转
